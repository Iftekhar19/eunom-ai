...
const PromptBox = ({ setIsFirst, isFirst, setSideBarData }) => {
  ...
  return (
    <div className="w-full xs:py-2 md:py-6 flex flex-col items-center">
      <div
        className="flex items-start bg-[#FFFFFF] min-h-[60px] rounded-[30px] w-full py-[6px] pl-5 pr-[6px] gap-3"
        style={{ boxShadow: "0px 8px 24px -4px #00000014" }}
      >
        <PromptBoxIcon  className="inline-block mt-2"/>
        ...
        <button onClick={generateContent} className="flex justify-center items-center h-[48px] w-[48px] rounded-full promptIconBox mt-3 mr-2">
          <GenerateIcon />
        </button>
      </div> 
    </div>
